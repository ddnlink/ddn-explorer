...
class CountDown extends Component {
    ...
    clock = () => {
        ...
        if (ti < count) {
            ...
            this.setState({
                time: ti,
                timeText: ti +' ' + formatMessage({ id: "home.left_time" }),
            });
        } else {
            this.setState({
                time: 1,
                timeText: `1 ${formatMessage({ id: "home.left_time" })}`
            })
        }
    }
    render() {
        return (
            <span style={{ marginLeft: '28px', fontSize: '13px' }}>{this.state.timeText}</span>
        )
    }
}
